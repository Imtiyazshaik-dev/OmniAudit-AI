        ...
        <div className="p-6 rounded-2xl bg-zinc-900/80 border border-zinc-800 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono">
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800">
              ...
              <span className="text-base font-bold text-white mt-1 block">₹{(invoice.amounts?.subtotal || 0).toLocaleString('en-IN')}</span>
            </div>

            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800">
              <span className="text-zinc-400 block text-[10px] uppercase">CALCULATED CGST</span>
              <span className="text-base font-bold text-indigo-400 mt-1 block">₹{(invoice.calculatedTaxSplit?.cgst || 0).toLocaleString('en-IN')}</span>
            ...
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800">
              ...
              <span className="text-base font-bold text-purple-400 mt-1 block">₹{(invoice.calculatedTaxSplit?.sgst || 0).toLocaleString('en-IN')}</span>
            ...
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800">
              ...
              <span className="text-base font-bold text-blue-400 mt-1 block">₹{(invoice.calculatedTaxSplit?.igst || 0).toLocaleString('en-IN')}</span>
          ...
          <div className="flex items-center justify-between pt-3 text-xs font-mono font-bold text-white border-t border-zinc-800/80">
            <span className="text-sm">GRAND TOTAL INVOICE AMOUNT:</span>
            <span className="text-lg text-emerald-400">₹{(invoice.amounts?.grandTotal || 0).toLocaleString('en-IN')}</span>
          </div>
        </div>

        {/* Line Items Table */}
        <div className="space-y-3">
          <h4 className="text-xs font-bold text-zinc-300 font-mono uppercase tracking-wider">EXTRACTED INVOICE LINE ITEMS</h4>
